...
import { Phone, Menu, X, Wifi, Tv, FileText, MessageSquare, ArrowRight, Info } from 'lucide-react';
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 backdrop-blur-md shadow-sm py-3' : 'bg-transparent py-5'}`} role="banner">
      <div className="container-custom flex items-center justify-between">
        {/* Logo with Popover and Info Button */}
            ...
            <PopoverTrigger asChild>
              <div className="flex items-center">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <button 
                      className="ml-2 text-poda-blue hover:text-poda-orange transition-colors" 
                      aria-label="Informace o PODA připojení"
                    >
                      <Info size={18} />
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="z-50 w-72 p-4 bg-white rounded-md shadow-lg">
                    <div className="space-y-3">
                      <h3 className="font-semibold text-poda-blue">PODA připojení</h3>
                      <p className="text-sm text-gray-600">
                        Jako autorizovaný obchodní zástupce PODA vám garantuji nejrychlejší internetové připojení za nejlepší cenu na trhu.
                      </p>
                      <p className="text-xs text-gray-500 mt-2">
                        Milan Terč | Kontaktní osobní zástupce PODA
                      </p>
                      <div className="pt-2">
                        <Link 
                          to="/kontakt" 
                          className="text-sm text-poda-blue hover:text-poda-orange flex items-center"
                        >
