
import { BlogPost } from './types';

export const polanka60ghzPost: BlogPost = {
  id: 201,
  title: "PODA 2025: Polanka nad Odrou připojena revolučním 60 GHz připojením",
  slug: "polanka-nad-odrou-60ghz-pripojeni-2025",
  excerpt: "Novinky PODA 2025! Obec Polanka nad Odrou u Ostravy získala nejmodernější bezdrátové připojení s 60 GHz technologií. Rychlost až 1000/200 Mbps prostřednictvím vzdušného připojení.",
  content: `
    <h2>PODA 2025: Revoluce v připojení Polanky nad Odrou</h2>
    
    <p>Společnost PODA v rámci projektu <strong>PODA 2025</strong> úspěšně připojila obec Polanka nad Odrou k nejmodernější 60 GHz bezdrátové technologii. Tato inovativní technologie přináší obyvatelům rychlé a spolehlivé internetové připojení přímo do jejich domovů.</p>
    
    <h3>60 GHz technologie - budoucnost je zde</h3>
    <p>Nová <strong>60 GHz technologie</strong> představuje průlom v oblasti bezdrátového připojení:</p>
    <ul>
      <li><strong>Ultrarychlá rychlost:</strong> Až 1000 Mbps download / 200 Mbps upload</li>
      <li><strong>Nízká latence:</strong> Ideální pro gaming a videokonference</li>
      <li><strong>Vysoká kapacita:</strong> Podporuje více zařízení současně</li>
      <li><strong>Stabilní připojení:</strong> Minimální výpadky díky pokročilé technologii</li>
    </ul>
    
    <h3>Výhody vzdušného připojení v Polance nad Odrou</h3>
    <p>Bezdrátové připojení přináší obyvatelům Polanky nad Odrou řadu výhod:</p>
    <ul>
      <li>Rychlá instalace bez nutnosti kopání</li>
      <li>Šetrné k životnímu prostředí</li>
      <li>Konkurenceschopné ceny</li>
      <li>Profesionální technická podpora PODA</li>
    </ul>
    
    <h3>Polanka nad Odrou - strategická lokalita</h3>
    <p>Polanka nad Odrou, ležící v těsné blízkosti Ostravy, se stala pilotním projektem pro nasazení 60 GHz technologie. Díky své poloze a rostoucí poptávce po rychlém internetu byla tato obec vybrána jako ideální místo pro testování nejmodernějších technologií.</p>
    
    <h3>Jak se připojit?</h3>
    <p>Zájemci z Polanky nad Odrou a okolí mohou kontaktovat PODA pro:</p>
    <ul>
      <li>Bezplatnou konzultaci a návrh řešení</li>
      <li>Posouzení možnosti připojení na konkrétní adrese</li>
      <li>Výběr nejvhodnějšího tarifa</li>
      <li>Profesionální instalaci</li>
    </ul>
    
    <h3>PODA 2025 - více lokalit v přípravě</h3>
    <p>Úspěšné nasazení 60 GHz technologie v Polance nad Odrou je součástí ambiciózního plánu PODA 2025, který předpokládá rozšíření této technologie do dalších obcí v Moravskoslezském kraji.</p>
    
    <h3>Kontakt a objednávka</h3>
    <p>Pro více informací o 60 GHz připojení v Polance nad Odrou kontaktujte PODA na <strong>730 431 313</strong> nebo navštivte naše pobočky v Ostravě.</p>
  `,
  date: "19. 08. 2025",
  author: "Odborník na telekomunikace",
  category: "Novinky",
  image: "/lovable-uploads/0c952940-aa5d-4157-b3a8-82b62d2a048c.png",
  alt: "60 GHz bezdrátová anténa PODA SUPER 2025 pro internetové připojení v Polance nad Odrou",
  tags: ["PODA 2025", "Polanka nad Odrou", "60 GHz", "bezdrátové připojení", "Ostrava", "novinky", "rychlý internet", "vzdušné připojení"]
};
